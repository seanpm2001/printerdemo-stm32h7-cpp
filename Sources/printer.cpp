...
struct InkLevelModel : slint::Model<InkLevel> {
  ...
  std::vector<InkLevel> m_data = {
      {slint::Color::from_rgb_uint8(255, 255, 0), 0.9},
...
void run_printer() {
  ...
  for (size_t i = 0; i < default_queue->row_count(); ++i) {
  ...
  // workaround the fact that there is no interrupt on touch by polling every
  // 30ms
  //    slint::Timer t(std::chrono::milliseconds(30), [] {});
